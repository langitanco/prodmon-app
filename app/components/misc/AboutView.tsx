...
export default function AboutView() {
  ...
  return (
    <div className="flex justify-center items-start min-h-full p-4 md:p-8">
      <div className="w-full max-w-2xl bg-white p-6 md:p-8 rounded-2xl shadow-lg border border-slate-200 space-y-6">
        
        {/* HEADER APLIKASI */}
        <div className="text-center border-b border-slate-100 pb-6">
          <div className="bg-blue-600 w-16 h-16 rounded-full flex items-center justify-center mx-auto mb-3 shadow-md shadow-blue-500/50">
            <Zap className="w-8 h-8 text-white" />
          </div>
          <h2 className="text-3xl font-extrabold text-slate-800">{APP_INFO.name}</h2>
          <p className="text-slate-500 text-sm mt-1">{APP_INFO.purpose}</p>
        </div>

        {/* INFO DASAR */}
        <div className="space-y-4">
          <h3 className="text-lg font-bold text-slate-700 border-b border-dashed border-slate-200 pb-2 flex items-center gap-2">
            <Package className="w-5 h-5 text-slate-400" /> Detail Aplikasi
          </h3>
        ...
        <div className="pt-4 space-y-4">
          <h3 className="text-lg font-bold text-slate-700 border-b border-dashed border-slate-200 pb-2 flex items-center gap-2">
            <User className="w-5 h-5 text-slate-400" /> Pengembang
          </h3>
          <div className="grid grid-cols-2 gap-y-3 text-sm">
        ...
        <div className="pt-4">
          ...
          >
            ...
            {showChangelog ? <ChevronUp className="w-5 h-5 text-slate-600" /> : <ChevronDown className="w-5 h-5 text-slate-600" />}
          </button>

          {showChangelog && (
            <div className="mt-4 space-y-6 animate-in fade-in duration-300">
              {CHANGELOG.map((log, index) => (
                <div key={log.version} className={`p-4 rounded-xl border ${index === 0 ? 'bg-blue-50 border-blue-200 shadow-md' : 'bg-white border-slate-200'}`}>
                  <div className="flex justify-between items-start mb-2">
                    <span className={`font-extrabold text-lg ${index === 0 ? 'text-blue-700' : 'text-slate-800'}`}>
                        v{log.version}
                    </span>
                    <span className={`text-xs font-medium px-2 py-0.5 rounded-full ${index === 0 ? 'bg-blue-600 text-white' : 'bg-slate-200 text-slate-600'}`}>
                        {log.date}
                    </span>
                  </div>

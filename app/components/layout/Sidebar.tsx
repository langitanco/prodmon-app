...
export default function Sidebar({ sidebarOpen, setSidebarOpen, currentUser, activeTab, handleNav, onLogout, onOpenProfile }: SidebarProps) {
  ...
  const menuGroups = [
    ...
    },
    {
      title: 'LAINNYA',
      items: [
        { id: 'settings', label: 'Pengaturan Admin', icon: Settings, visible: p?.settings?.view },
    ...
    <>
      ...
      `}>
          ...
          <div className="flex items-center gap-3">
            <div className="w-12 h-12 rounded-xl flex items-center justify-center overflow-hidden">
              <img
                ...
                loading="lazy"
                onError={(e) => { e.currentTarget.src = 'https://ui-avatars.com/api/?name=L&background=0066FF&color=fff'; }}
              />
            </div>
        ...
        <div className="flex-1 overflow-y-auto px-4 py-2 space-y-6 custom-scrollbar">
          <ProfileCardMobileMemo currentUser={currentUser} onOpenProfile={onOpenProfile} />
          {menuGroups.map((group, idx) => (
            <div key={idx}>
              <h3 className="text-[10px] font-extrabold text-slate-400 dark:text-slate-500 uppercase tracking-wider mb-3 px-3">
                {group.title}
              </h3>
        ...
        <ProfileCardDesktopMemo currentUser={currentUser} onOpenProfile={onOpenProfile} onLogout={onLogout} />
      </aside>
    </>
  );
}
